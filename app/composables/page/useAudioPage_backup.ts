...
export function useAudioPage() {
  ...
  const startDeviceReconnectionMonitoring = () => {
    if (pageState.loading.isReconnecting) return
  ...
  const handleDeviceReconnection = async () => {
    ...
    try {
      ...
      handleAudioError('device_disconnected', error as Error)
    }
  }

  // 檢測並處理新設備（可能是重新插入的設備）
  const detectAndHandleNewDevices = async () => {
    try {
      const devices = await navigator.mediaDevices.enumerateDevices()
  ...
  // 處理錄音過程中的設備斷線
  const handleRecordingDeviceDisconnection = async () => {
    ...
    // 3. 顯示用戶友善的錯誤提示
    pageState.errors.deviceError = '⚠️ 麥克風設備已斷線，正在等待重新連接...'
    
    // 4. 啟動設備重連監控
    pageState.data.reconnectAttempts = 0
    startDeviceReconnectionMonitoring()
    
    // 5. 創建更詳細的錯誤記錄
    const disconnectionError = createAudioError(
      'device_disconnected',
      new Error('錄音過程中設備斷線'),
      [
        '請重新連接您的麥克風設備',
        '系統會自動檢測設備重連並恢復錄音',
        '如果問題持續，請檢查設備連線或選擇其他麥克風'
      ]
    )
    pageState.errors.lastError = disconnectionError
  }
  ...
  // 處理錯誤並更新狀態
  const handleAudioError = (type: AudioErrorType, error: Error | DOMException) => {
          ...
          try {
            ...
            if (!deviceStillAvailable) {
              ...
              if (pageState.data.currentSession?.status === 'recording') {
                console.log('🚨 錄音過程中檢測到設備斷線，暫停錄音')
                
                // 暫停錄音
                if (pageState.data.currentSession.mediaRecorder?.state === 'recording') {
                  pageState.data.currentSession.mediaRecorder.pause()
                  pageState.data.currentSession.status = 'paused'
                  stopDurationTimer()
                }
                ...
                // 更新狀態和錯誤訊息
                pageState.data.deviceConnectionStatus = 'disconnected'
                pageState.errors.deviceError = '⚠️ 麥克風設備已斷線，請重新連接設備以繼續錄音'
                ...
                handleAudioError('device_disconnected', new Error('設備已斷線'))
              }
  ...
  const finalizeRecording = () => {
    ...
    // 計算持續時間
    const duration = pageState.data.currentSession.endTime && pageState.data.currentSession.startTime
      ? (pageState.data.currentSession.endTime - pageState.data.currentSession.startTime) / 1000
      : 0
  ...
  const stopRecording = () => {
    ...
    if (pageState.data.currentSession.startTime) {
      pageState.data.currentSession.duration
        = (pageState.data.currentSession.endTime - pageState.data.currentSession.startTime) / 1000
    }
